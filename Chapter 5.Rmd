...
transformed_response <- log(df$abundance + 1)
hist(transformed_response, main = "Histogram of Transformed Response Variable")
shapiro.test(transformed_response)
qqnorm(transformed_response)
boxplot(transformed_response, main = "Boxplot of Transformed Response Variable")

log_data <- df
log_data$log_abundance <- transformed_response

log_model <- lm(transformed_response ~ microsite, data = log_data)
summary(log_model)

exp(coef(log_model))
exp(confint(log_model))

plot(predict(log_model), residuals(log_model),
...
abundance_anova_result <- aov(abundance ~ microsite*site_code + mean_ambient_temp + shrub_density, data = df) ### Ground temp significantly varied
print(summary(abundance_anova_result))
```
...
richness_anova_result <- aov(richness ~ microsite*site_code + mean_ambient_temp + shrub_density, data = df) ### Ground temp significantly varied
print(summary(richness_anova_result))

richness_emmeans_result <- emmeans(richness_anova_result, pairwise ~ microsite)
