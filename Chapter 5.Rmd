...
df <- merge(photo, tidy_20m, by = c("site_code", "factor", "microsite_number"))
colnames(df)[4] <- "rep"
colnames(df)[5] <- "abundance"
colnames(df)[9] <- "shrub_density"

df <- df %>%
  dplyr::select(-rep.y, -functioned)
df <- df %>%
...
ggplot(df, aes(x = factor, y = abundance)) +
  geom_boxplot() +
  labs(
       x = "Factor",
...
model1 <- glm(abundance~factor + mean_ambient_temp + shrub_density, family = "poisson", data = df)
model1
anova(model1, test = "Chisq")
abundance_anova_result <- aov(abundance ~ factor + mean_ambient_temp + shrub_density, data = df) ### Ground temp significantly varied
